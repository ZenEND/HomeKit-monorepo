...
export function AIAssistPanel({ onGenerated }: AIAssistPanelProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { data } = await apiInstance.post<CardFormData>('/ai/generate-card-fields', {
        systemPrompt,
        userPrompt: prompt,
      });
      ...
      if (message?.includes('No AI model is configured')) {
        setError('No AI model is configured. Ask your admin to set up an API key.');
      } else {
