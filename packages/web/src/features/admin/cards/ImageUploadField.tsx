...
import { fileViewUrl, uploadGameImage } from '@/api/cards';
...
export function ImageUploadField({ value, onChange, folderId }: ImageUploadFieldProps) {
  ...
  const handleCropSave = async () => {
    ...
    try {
      ...
      const { id } = await uploadGameImage(file, folderId);
      onChange(id);
      setRawSrc(null);
    } catch {
  ...
  const previewUrl = value ? fileViewUrl(value) : undefined;
    ...
    <div className="flex flex-col gap-3">
      ...
      {previewUrl ? (
        <div className="relative overflow-hidden rounded-xl border border-secondary/60" style={{ aspectRatio: '2/3', maxWidth: 120 }}>
          <img src={previewUrl} alt="Card" className="h-full w-full object-cover" />
          <button
            type="button"
